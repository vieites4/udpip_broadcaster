...
void cb_forward_recvfrom(public_ev_arg_r *arg)
{
	...
	if(memcmp(HT,tsb0,1)==0&& (lon_int>1)){
		...
		if (error==0 && duplicate_control(datos,arg->locT)==1){
			pkt = TSB_f(datos,arg->gn);aa=1; 	memcpy(tx_frame1->buffer.data,(char *)  pkt, IEEE_80211_BLEN);print_hex_data(&tx_frame1->buffer,arg->len);printf("\n");
			send_message(	(sockaddr_t *)arg->forwarding_addr,arg->forwarding_socket_fd,&tx_frame1->buffer, arg->len	);
			PRF("saio de tsb_f \n");
			}
		...
		send_message(	(sockaddr_t *)arg->forwarding_addr,arg->forwarding_socket_fd,&tx_frame1->buffer, arg->len	);
		PRF("envio realizado\n");print_hex_data(&tx_frame1->buffer,arg->len);printf("\n");
		...
		if (error==0 && duplicate_control(datos,arg->locT)==1){
			...
			int y =geo_limit(HT,pkt);memcpy(tx_frame1->buffer.data,(char *)  pkt, IEEE_80211_BLEN);print_hex_data(&tx_frame1->buffer,arg->len);printf("\n");
			if (y>=0){	send_message(	(sockaddr_t *)arg->forwarding_addr,arg->forwarding_socket_fd,&tx_frame1->buffer, arg->len	);}
