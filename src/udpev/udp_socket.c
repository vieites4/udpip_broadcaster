...
const unsigned char ETH_ADDR_BROADCAST[ETH_ALEN] = { 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFf };
const unsigned char ETH_ADDR_ANY[ETH_ALEN] = { 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00 };
const unsigned char ETH_ADDR_FAKE[ETH_ALEN] = { 0x00, 0x18, 0x39, 0xAE, 0x7D,
		0xD5 };
const unsigned char ETH_ADDR_NULL[ETH_ALEN] = { 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00 };
...
int get_mac_address(const int socket_fd, const char *if_name,
...
/* init_ifreq */
ifreq_t *init_ifreq(const char *if_name) {//si
	...
	if (if_name_len > ifr_name_len) {
		handle_app_error(
				"init_ifreq: if_name's length bigger than ifr's buffer.\n");
	}
...
/* new_sockaddr_in */
sockaddr_in_t *new_sockaddr_in() { //si
...
PRF("entrei en init_sockaddr_ll\n");
...
sockaddr_in_t *init_if_sockaddr_in(const char *if_name, const int port) {
	...
	if (getifaddrs(&ifaddr) < 0) {
		handle_sys_error(
				"init_if_sockaddr_in: <getifaddrs> returned error. Error: ");
	}
	...
	for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
		...
		i = getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in),
				(char *) host, NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
		if ((strcmp(ifa->ifa_name, if_name) == 0)
				&& (ifa->ifa_addr->sa_family == AF_INET)) {
			if (i != 0) {
				handle_app_error(
						"init_if_sockaddr_in: <getnameinfo> failed: %s\n",
						gai_strerror(i));
			}
...
int open_receiver_udp_socket(const int port) {
	...
	if ((fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
		handle_sys_error(
				"open_receiver_udp_socket: <socket> returns error. Description");
	}
	...
	if (bind(fd, (sockaddr_t *) addr, LEN__SOCKADDR_IN) < 0) {
		handle_sys_error(
				"open_receiver_udp_socket: <bind> returns error. Description");
	}

	// 3) for analyzing received message's headers
	if (set_msghdrs_socket(fd) < 0) {
		handle_app_error(
				"open_receiver_udp_socket: <set_msghdrs_socket> returns error.\n");
	}
	return (fd);
...
int open_receiver_raw_socket(const int port) {
	...
	if ((fd = socket(PF_PACKET, SOCK_RAW, port)) < 0)	// lsap é port
			{
		handle_sys_error(
				"open_receiver_RAW_socket: <socket> returns error. Description");
	}
	// 2) local address for binding
...
handle_app_error(
		"open_broadcast_udp_socket: <set_broadcast_socket> returns error.\n");
}
	// 3) broadcast socket must be bound to a specific network interface
if (set_bindtodevice_socket(iface, fd) < 0) {
handle_app_error(
		"open_broadcast_udp_socket: <set_bindtodevice_socket> returns error.\n");
}
...
ifreq_t *ifr = new_ifreq();
if (setsockopt(socket_fd, IPPROTO_IP, IP_PKTINFO, &ifr, LEN__IFREQ) < 0) {
...
int set_ll_frame(ll_frame_t *frame, const int frame_type, const int frame_len) {
...
ieee80211_frame_t *init_ieee80211_frame(const int ll_sap,
const unsigned char *h_dest, const unsigned char *h_source) {
...
int sent_bytes = 0;
if ((sent_bytes = sendto(socket_fd, buffer, len, 0, dest_addr, LEN__SOCKADDR_LL))
	< 0) {
...
/* recv_msg */
int recv_msg(const int socket_fd, msg_header_t *msg, const in_addr_t block_ip,
bool *blocked) {
int rx_bytes = 0;
