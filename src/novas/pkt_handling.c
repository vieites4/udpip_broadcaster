...
itsnet_packet * TSB(void *buffer, List_lsp *lsp, List_lsp *rep){
	...
	memcpy((void *)&ch->nh_reserved,(char *)(buffer) +7,1);
	...
	memcpy(bh->lt,(void *)lt,1);free(lt);
	...
	char *str3[9] = {'\0'}; //esto non sei eu!
	sprintf(str3,"%08X",hl_int-1);
	memcpy(bh->rhl,(void *)str3,1);
	itsnet_tsb_t *tsb_h;
	tsb_h=(itsnet_tsb_t *)malloc(sizeof(itsnet_tsb_t));
	...
	//print_hex_data(pkt->payload.itsnet_tsb.payload.payload,lon_int);printf(" .\n");
	pkt->common_header=*ch;
	pkt->basic_header=*bh;free(ch);free(bh);
	//NEIGHBOURS.
	if (memcmp((char *)(buffer) +7,tipoa,1)==0)
	{		memcpy(tsb_h->payload.btp1,(char *)(buffer) + 18,2);
	memcpy(tsb_h->payload.btp2,(char *)(buffer) + 16,2);
	}else{
		char info_dest[2];
		memset(info_dest,0,2);
		memcpy(tsb_h->payload.btp1,(char *)(buffer) + 18,2);
		memcpy(tsb_h->payload.btp2,info_dest,2);}
	pkt->payload.itsnet_tsb=*tsb_h;free(tsb_h);
...
itsnet_packet * SHB(void *buffer, List_lsp *lsp, List_lsp *rep){
	...
	itsnet_shb_t *shb_h;
	shb_h=(itsnet_shb_t *)malloc(sizeof(itsnet_shb_t));
	...
	//print_hex_data(pkt->payload.itsnet_shb.payload.payload,lon_int);printf(" .\n");
	pkt->common_header=*ch;
	pkt->basic_header=*bh;free(ch);free(bh);
	//NEIGHBOURS.
	if (memcmp((char *)(buffer) +7,tipoa,1)==0)
	{		memcpy(shb_h->payload.btp1,(char *)(buffer) + 18,2);
	memcpy(shb_h->payload.btp2,(char *)(buffer) + 16,2);
	}else{
		char info_dest[2];
		memset(info_dest,0,2);
		memcpy(shb_h->payload.btp1,(char *)(buffer) + 18,2);
		memcpy(shb_h->payload.btp2,info_dest,2);		}
	pkt->payload.itsnet_shb=*shb_h;free(shb_h);
	if  ((locT_general->len== 0 || any_neighbours()==0)&& ch->traffic_class.scf==1){
		itsnet_packet * pkt1 = NULL;
...
itsnet_packet * GeoUnicast(void *buffer, List_lsp *lsp, List_lsp *rep,public_ev_arg_r *arg){
	...
	memcpy((void *)&LPV_dest->node_id,(char *)(buffer)+16,8);
	...
	if(val!=0){
		...
		if  (search_in_locT_m_pending (LPV_dest->node_id.mac,locT_general)!=0 ){  //&& ch.traffic_class.scf==1
			...
			int i =add_end_lsp(ls_buffer, *pkt,3);free(pkt);	free(LPV_dest);
			return(pkt1);
		}else{
			int pos_loct=search_in_locT_m(LPV_dest->node_id.mac,locT_general);free(LPV_dest);
			if ( pos_loct==0 && ch->traffic_class.scf==1){
				...
				memcpy((void *)&un_h.dest_position_vector,position,20);
				...
				if(itsGnGeoUnicastForwardingAlgorithm==0||itsGnGeoUnicastForwardingAlgorithm==1){
					...
					if (memcmp(nh.address,TWOS,6)==0 ||memcmp(nh.address,ZEROS,6)==0){	free(pkt);return(pkt_lsp);}else{
						...
						send_message(	(sockaddr_t *)dir,arg->forwarding_socket_fd,&tx_frame->buffer,lon_int+ 48+4+8+14+4);free(tx_frame);free(pkt);free(dir);return(pkt_lsp);
					}				}
				else if(itsGnGeoUnicastForwardingAlgorithm==2){
					CBF_UC(pkt,lon_int+ 48+4+8+14+4,LPV);	free(pkt);
					return(pkt_lsp);
				}			}}
	}else{
		free(LPV_dest);
		...
		memcpy((void *)&ls_req.GN_ADDR,(char *)(buffer)+16,8);
		...
		AddTimer(ls_req.sequencenumber,3*itsGnBeaconServiceRetransmitTimer,5);
		data->SN1=ls_req.sequencenumber;
		add_end_locT(locT_general,*data);free(data);
		//ls retransmisor
		...
		add_end_lsp(ls_buffer, *pkt,3);free(pkt_ls);	}
...
itsnet_packet * GeoBroadcast(void *buffer, List_lsp *lsp, List_lsp *rep,public_ev_arg_r *arg){
	...
	memcpy((void *)&ch->traffic_class,(char *)(buffer) +6,1);
	memcpy(ch->flags,(char *)(buffer) +3,1);
	memcpy(ch->mhl,(char *)(buffer) +2,1);//print_hex_data(ch->mhl,1);PRF(" mHL dentro de geobroad\n");
	int hl_int=sprint_hex_data( (char *)(buffer) +2, 1);
	memcpy((void *)&ch->HT_HST,(char *)(buffer),1);
	memset(ch->reserved,0,1);
	memcpy((void *)&ch->nh_reserved,(char *)(buffer) +7,1);
	ch->nh_reserved.HT=ch->nh_reserved.HST;
	ch->nh_reserved.HST=0;
	...
	//char *str3[9] = {'\0'};
	//sprintf(str3,"%08X",hl_int-1);
	memcpy(bh->rhl,ch->mhl,1);//print_hex_data(bh->rhl,1);PRF(" %d \n",sizeof(itsnet_geobroadcast_t));
	itsnet_geobroadcast_t *gbc_h=NULL;
	gbc_h=(itsnet_geobroadcast_t *)malloc(sizeof(itsnet_geobroadcast_t));
//	pkt->payload.itsnet_geobroadcast=(itsnet_geobroadcast_t *)malloc(sizeof(itsnet_geobroadcast_t));
	...
	memset(gbc_h->reserved2,0,2);//(char *)(buffer) +30
	memcpy(gbc_h->payload.payload,(char *)(buffer) +36,lon_int);
	//print_hex_data(gbc_h->dest_latitude,4);printf (" latitude \n");
//		print_hex_data(gbc_h->dest_longitude,4);printf (" longitude \n");

	//print_hex_data(pkt->payload.itsnet_geobroadcast.payload.payload,lon_int);printf(" .\n");
	pkt->common_header=*ch;
	pkt->basic_header=*bh;free(bh);
	//NEIGHBOURS.
	if (memcmp((char *)(buffer) +7,tipoa,1)==0)
	{		memcpy(gbc_h->payload.btp1,(char *)(buffer) + 34,2);
	memcpy(gbc_h->payload.btp2,(char *)(buffer) + 32,2);
	}else{
		char info_dest[2];
		memset(info_dest,0,2);
		memcpy(gbc_h->payload.btp1,(char *)(buffer) + 34,2);
		memcpy(gbc_h->payload.btp2,info_dest,2);		}
	pkt->payload.itsnet_geobroadcast=*gbc_h;free(gbc_h);

//	printf ("**************************** ");print_hex_data(&pkt->payload.itsnet_geobroadcast.source_position_vector,sizeof(itsnet_position_vector));printf (" \n");
	...
	if  ((locT_general->len== 0 || any_neighbours()==0) && ch->traffic_class.scf==1){
		...
		int i =add_end_lsp(lsp_bc_g, *pkt,0);free(pkt);free(ch);
		return(pkt1);		//buffer in BC AND omit next executions
	}
	free(ch);
	mac_addr nh;
...
//	memset(pkt->payload.itsnet_geobroadcast.reserved,0,2);//(char *)(buffer) +30
	...
	if(itsGnGeoBroadcastForwardingAlgorithm==0||itsGnGeoBroadcastForwardingAlgorithm==1){
		...
		if (f>=0){
			...
			memcpy(tx_frame->buffer.header.type,type,2);
			memcpy(tx_frame->buffer.data, pkt, IEEE_80211_BLEN);
			send_message(	(sockaddr_t *)dir,arg->forwarding_socket_fd,&tx_frame->buffer,lon_int+ 44+4+8+14+4+4);
			//print_hex_data(&pkt->payload.itsnet_geobroadcast.payload,lon_int+4);PRF("\n");
			free(dir);free(pkt);free(tx_frame);return(pkt1);}

		else{PRF("IT'S VERY FAR!!\n");
		if(LPV->pai==1){
			int f1=geo_limit_ll(pkt,LPV);
			if (f1>=0){
				...
				if (memcmp(nh.address,TWOS,6)==0 ||memcmp(nh.address,ZEROS,6)==0){free(pkt);return(pkt1);}else{
					ieee80211_frame_t *tx_frame = init_ieee80211_frame(arg->forwarding_port, nh.address,h_source);
					char type[2]={0x07,0x07};
					memcpy(tx_frame->buffer.header.type,type,2);
					memcpy(tx_frame->buffer.data, pkt, IEEE_80211_BLEN);
					send_message(	(sockaddr_t *)dir,arg->forwarding_socket_fd,&tx_frame->buffer,lon_int+ 44+4+8+14+4);free(dir);free(tx_frame);free(pkt);return(pkt1);
					//print_hex_data(&tx_frame->buffer,lon_int+ 44+4+8+4);PRF("\n");
				}}else{
					//discard
					free(pkt);free(dir);
					return(pkt1);	}			}else{free(dir);
						return(pkt); //broadcast
					}		}	}
	else if(itsGnGeoBroadcastForwardingAlgorithm==2){

		nh=CBF_BC(pkt,lon_int+ 44+4+8+14+4,LPV); //first send PV_SE is LPV
		if (memcmp(nh.address,TWOS,6)==0 ||memcmp(nh.address,ZEROS,6)==0){free(pkt);return(pkt1);}else{
		...
		nh=	Advanced_BC(pkt,lon_int+ 44+4+8+14+4,LPV,NULL,h_source);
		if (memcmp(nh.address,TWOS,6)==0 ||memcmp(nh.address,ZEROS,6)==0){free(pkt);free(dir);return(pkt1);}else{
...
itsnet_packet *  GeoAnycast(void *buffer, List_lsp *lsp, List_lsp *rep,public_ev_arg_r *arg){
	itsnet_packet * pkt = NULL;
	pkt=(itsnet_packet *)malloc(sizeof(itsnet_packet));
	itsnet_common_header *ch=NULL;
	itsnet_basic_header *bh=NULL;
	...
	if(y>=0){
		if(itsGnGeoBroadcastForwardingAlgorithm==0||itsGnGeoBroadcastForwardingAlgorithm==1){
			...
			nh=CBF_BC(pkt,lon_int+ 44+4+8+14+4,LPV); //first send PV_SE is LPV
			if (memcmp(nh.address,TWOS,6)==0 ||memcmp(nh.address,ZEROS,6)==0){free(dir);free(pkt);return(pkt1);}else{
			...
			if (memcmp(nh.address,TWOS,6)==0 ||memcmp(nh.address,ZEROS,6)==0){free(dir);free(pkt);return(pkt1);}else{
				...
				if(itsGnGeoUnicastForwardingAlgorithm==0||itsGnGeoUnicastForwardingAlgorithm==1){
					...
					if (memcmp(nh.address,TWOS,6)==0 ||memcmp(nh.address,ZEROS,6)==0){free(dir);free(pkt);return(pkt1);}else{
						...
						memcpy(tx_frame->buffer.header.type,type,2);
						memcpy(tx_frame->buffer.data, pkt, IEEE_80211_BLEN);
						send_message(	(sockaddr_t *)dir,arg->forwarding_socket_fd,&tx_frame->buffer,lon_int+ 44+4+8+14+4);free(tx_frame);free(pkt);free(dir);return(pkt1);
					}				}
				else if(itsGnGeoUnicastForwardingAlgorithm==2){
					CBF_UC(pkt,lon_int+ 44+4+8+14+4,LPV);free(dir);
					return(pkt1);
				}
...
int BasicHeader_processing(public_ev_arg_r *arg){
	...
	ht_hst_t VERSION;
	memcpy((void *)&VERSION,buffer+14,1);
	int error=0;
	if (VERSION.HT!=itsGnProtocolVersion){PRF("version!\n");return(1);}
	if (itsIfType==1){PRF("Media-dependent funcionalities for ITS-G5\n");}
	if (VERSION.HST!=2){error =CommonHeader_processing(arg);PRF("error %d\n",error);}else{ PRF("SECURE!\n");}
	return(error);
...
int CommonHeader_processing(public_ev_arg_r *arg){
	...
	if (rhl>mhl){return(1);} //poñer unha solucion a esto!
	...
	//data->pdr=1/(PDR);PRF("%d no common processing\n",data->pdr);PRF("%d %dno common processing\n",PDR,PDR_ini);
	...
	if(memcmp(HT1,geounicast,1)==0 && memcmp(DE_LPV+2,h_source,6)!=0){
		if(NH->HST==0 ||NH->HST==1 ){
			...
			memcpy((void *)&data1->pos_vector,DE_LPV,20);
		...
		while(pos1!=NULL){
			if(memcmp((char *)&pos1->data.common_header.HT_HST,geounicast,1)==0){
				if(memcmp(pos1->data.payload.itsnet_unicast.dest_position_vector.node_id.mac.address,data->mac_id.address,6)==0){
					...
					lt->base=num3;lt->multiple=num4;
					memcpy(pos1->data.basic_header.lt,(void *)lt,1);free(lt);
		...
		while(pos0!=NULL)
			if(memcmp((char *)&pos0->data.common_header.HT_HST,geounicast,1)==0){
				if(memcmp(pos0->data.payload.itsnet_unicast.dest_position_vector.node_id.mac.address,data->mac_id.address,6)==0){
					...
					num3=strtol(str1,NULL,16);//PRF("%d\n", num3);
					lt->base=num3;lt->multiple=num4;
					memcpy(pos0->data.basic_header.lt,(void *)lt,1);free(lt);lt=NULL;
	...
	free(data);data=NULL;PRF("saio de common header processing\n");
	return(0);
...
itsnet_packet_f * TSB_f(void *buffer){
	...
	memcpy((void *)&pkt->common_header.traffic_class,(char *)(buffer) +6,1);
...
itsnet_packet_f * SHB_f(void *buffer){
	...
	pkt->common_header.btp.HT=pkt->common_header.btp.HST;
	pkt->common_header.btp.HST=0;
	//print_hex_data(&pkt->common_header.btp,1);PRF(" o do btp\n");
	itsnet_position_vector * PV=NULL;//
	PV= (itsnet_position_vector *)malloc(sizeof(itsnet_position_vector));
...
itsnet_packet_f * GeoBroadcast_f(void *buffer){
	...
	pkt=(itsnet_packet_f *)malloc(sizeof(itsnet_packet_f));
	memcpy((void *)&pkt->common_header.btp,(char *)(buffer),1);
	memset(pkt->common_header.altitude,0,2);
	memset(pkt->common_header.accuracies,0,2);
	pkt->common_header.btp.HT=pkt->common_header.btp.HST;
	pkt->common_header.btp.HST=0;
	itsnet_position_vector * PV=NULL;//
	PV= (itsnet_position_vector *)malloc(sizeof(itsnet_position_vector));
...
itsnet_packet * LS_req_f(void *buffer){
	...
	ch=(itsnet_common_header *)malloc(sizeof(itsnet_common_header));
	bh=(itsnet_basic_header *)malloc(sizeof(itsnet_basic_header));
	memcpy((void *)bh,(char *)(buffer),4);
	memcpy((void *)ch,(char *)(buffer) +4,8);
	memset(ch->mhl,0,1);
	ch->HT_HST.HT=2;
	ch->HT_HST.HST=1;
	...
	//PRF("SN %d\n",tsb_h.sequencenumber);
	SN_g++; //máximo SN?? % SN_MAX;
	memcpy((void *)&un_h.dest_position_vector,LPV,20);
	...
	pkt->payload.itsnet_unicast=un_h;//print_hex_data(pkt->payload.itsnet_tsb.payload.payload,lon_int);printf(" .\n");
	pkt->common_header=*ch;
	pkt->basic_header=*bh;free(bh);free(ch);free(LPV_so);free(lt);
